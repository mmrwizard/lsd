...
// 5. 将line1和line2中的每条直线段的参数输出到数组h（r, theta）
	...
	for (int i = 0; i < rows1; ++i) {
		...
		// 计算中点坐标
		float mid_x = (lines1[i](0)+lines1[i](3))/2-img1_width/2;
		float mid_y = (lines1[i](2) + lines1[i](4))/2 -img1_height/2;
	...
	for (int i = 0; i < rows2; ++i) {
		...
		// 计算中点坐标
		float mid_x = (lines2[i](0) + lines2[i](3))/2 - img2_width / 2;
		float mid_y = (lines2[i](2) + lines2[i](4))/2 - img2_height / 2;
	...
	for (int i = 0; i < rows1; i++) {
		...
		std::cout << min_det_theta << std::endl;

		if (min_det_theta_index != -1) {
	...
	for (size_t i = 0; i < match_lines.size(); i++) {
		...
		// 在线段中点添加数字标签
		Point midpoint((pt1.x + pt2.x) / 2, (pt1.y + pt2.y) / 2);
		cv::putText(matchedLines2, std::to_string(i + 1), midpoint, FONT_HERSHEY_SIMPLEX, 1, Scalar(0, 255, 0), 2);
	}
